...
@section('contenido')
    ...
    <div class="container">
        <h4 class="text-primary mb-3"><i class="fas fa-scroll me-2"></i>Registro de Acta de Defunción</h4>
        ...
        <form action="{{ route('defuncion.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <!-- Columna Izquierda: Datos Registrales -->
                <div class="col-md-6">
                    <div class="card h-100">
                        <h5 class="card-title mb-0 text-center bg-primary py-2 text-white">📄 Datos Registrales</h5>
                        <div class="card-body" style="height:70vh;overflow-y:auto">
                            ...
                            <div class="row mb-2">
                                <div class="col-6 mb-2">
                                    ...
                                    <input type="text" class="form-control bg-light" value="{{ $numeroFormateado }}" readonly>
                                </div>
                                <div class="col-6 mb-2">
                                    ...
                                    <div class="input-group">
                                        <select name="id_folio" id="id_folio" class="form-select @error('id_folio') is-invalid @enderror" required>
                                            ...
                                            @foreach($folios as $folio)
                                                <option value="{{ $folio->id_folio }}" {{ old('id_folio') == $folio->id_folio ? 'selected' : '' }}>
                            ...
                            <div class="mb-2">
                                ...
                                <div class="input-group">
                                    <input type="text" id="NombreFallecido" class="form-control @error('dni_fallecido') is-invalid @enderror" placeholder="Seleccione una persona" readonly required
                                           value="{{ old('dni_fallecido') ? ($personas->where('dni', old('dni_fallecido'))->first()->nombre_completo ?? '') : '' }}">
                                ...
                                <input type="hidden" name="dni_fallecido" id="dni_fallecido" value="{{ old('dni_fallecido') }}">
                                @error('dni_fallecido')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="row mb-2">
                                <div class="col-6 mb-2">
                                    ...
                                    <div class="input-group">
                                        <input type="date" name="fecha_defuncion" id="fecha_defuncion" 
                                            ...
                                            value="{{ old('fecha_defuncion') }}" required>
                                ...
                                <div class="col-6 mb-2">
                                    ...
                                    <textarea name="causa_defuncion" id="causa_defuncion" 
                                        ...
                                        rows="3" placeholder="Describa la causa...">{{ old('causa_defuncion') }}</textarea>
                            ...
                            <div class="row mb-2">
                                <div class="col-md-4 mb-2">
                                    ...
                                    <select id="region_defuncion" class="form-select" required>
                                        ...
                                        @foreach($regiones as $region)
                                            <option value="{{ $region->id_region }}" {{ old('region_defuncion') == $region->id_region ? 'selected' : '' }}>{{ $region->nombre }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-4 mb-2">
                                    <label for="provincia_defuncion" class="mb-2">Provincia</label>
                                    <select id="provincia_defuncion" class="form-select" required disabled>
                                ...
                                <div class="col-md-4 mb-2">
                                    ...
                                    <select name="id_distrito_def" id="id_distrito_def" 
                                        class="form-select @error('id_distrito_def') is-invalid @enderror" required disabled>
                            ...
                            <div class="mb-2">
                                ...
                                <input type="text" class="form-control bg-light" 
                                    value="{{ $usuarioLogueado ? $usuarioLogueado->persona->nombre_completo : '' }}" readonly>
                                <input type="hidden" name="id_usuario" value="{{ $usuarioLogueado ? $usuarioLogueado->id_usuario : old('id_usuario') }}">
                                @error('id_usuario')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="mb-2">
                                ...
                                <input type="text" class="form-control bg-light" 
                                    value="{{ $alcaldeUnico ? $alcaldeUnico->persona->nombre_completo : '' }}" readonly>
                                <input type="hidden" name="id_alcalde" value="{{ $alcaldeUnico ? $alcaldeUnico->id_alcalde : old('id_alcalde') }}">
                                @error('id_alcalde')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                            </div>
                ...
                <div class="col-md-6">
                    <div class="card h-100">
                        ...
                        <div class="card-body" style="height:70vh;overflow-y:auto">
                            <div class="mb-3">
                                <label class="mb-2">Acta (PDF)</label>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const cargarUbicacion = (url, select, callback = null) => {
            fetch(url).then(r => r.json()).then(data => {
                select.innerHTML = '<option value="">Seleccione</option>';
                data.forEach(item => {
                    const value = url.includes('provincias') ? item.id_provincia : item.id_distrito;
                    select.innerHTML += `<option value="${value}">${item.nombre}</option>`;
                });
        ...
        elements.region.addEventListener('change', function() {
            ...
            if (this.value) cargarUbicacion(`/provincias/${this.value}`, elements.provincia);
        });
        
        elements.provincia.addEventListener('change', function() {
            elements.distrito.disabled = true;
            if (this.value) cargarUbicacion(`/distritos/${this.value}`, elements.distrito);
        });
        ...
        @if(old('region_defuncion'))
            setTimeout(() => {
                elements.region.value = '{{ old('region_defuncion') }}';
                elements.region.dispatchEvent(new Event('change'));
                setTimeout(() => {
                    @if(old('provincia_defuncion'))
                        elements.provincia.value = '{{ old('provincia_defuncion') }}';
                        elements.provincia.dispatchEvent(new Event('change'));
                        setTimeout(() => {
                            @if(old('id_distrito_def'))
                                elements.distrito.value = '{{ old('id_distrito_def') }}';
                            @endif
                        }, 300);
                    @endif
                }, 300);
